...
- 云顶之弈优先使用 CommunityDragon 的 TFT live 数据文件；若该文件没有版本元数据，则回退到 CommunityDragon 版本索引。面板会展示版本、英雄/装备/羁绊/强化符文数量和部分名称预览，并提供 DataTFT、TFTable 的阵容统计入口。不再把英雄联盟 Data Dragon 的版本列表直接当作云顶版本。补丁公告按当前主版本号生成官方公告链接；例如 `16.17.1` 通常对应 `16.17` 补丁说明，小版本后缀可能不同。
